...
public class Fattura implements Entita{
	...
	String QUANTITA;
	String SOCIETA;
	String TESTO_POS_FATT;
	String TIPO_DES_CONT;
	String UM_ORDINE_ACQ;
	String USER_FATTURA;
	...
	public Fattura(Map<String,String> fattura) {
		...
		TESTO_POS_FATT = fattura.get("d4p1:TESTO_POS_FATT");
		TIPO_DES_CONT = fattura.get("d4p1:TIPO_DES_CONT");
		UM_ORDINE_ACQ = fattura.get("d4p1:UM_ORDINE_ACQ");
	...
	public String generaQuery() {
		...
		return "SELECT '"+ID_SIS_SORGENTE+"','"+NUM_FATTURA+"',"+ESERC_FATTURA+","+POS_FATTURA+",'"+NUM_MDA+"','"+DES_CONT_FATT+"','"+NUM_FATTURA_EST+"','"+FLAG_ADD_SUCC+"','"+DATA_REG_FATTURA+"',"+IMPORTO_POS_FATTURA+",'"+DIVISA_FATTURA+"',"+VAL_POS_FATT_EUR+",'"+NUM_DOC_ODL+"','"+POS_DOC_ODL+"','"+CAMBI_FATTURA+"','"+CAT_POS_DOC_ACQUISTI+"','"+CENTRO_DI_COSTO+"','"+CENTRO_DI_RESPONSABILITA+"','"+CONTO_CO_GE+"','"+CHIAVE_INTERNA_OGG_IMM+"','"+CONTROLLING_AREA+"','"+CREDITORE_DIFFERENTE+"','"+DATA_INSERIMENTO+"','"+ELEMENTO_WBS+"','"+FLAG_DARE_AVERE+"',"+IMPORTO_DIVISA_DOC+",'"+NUM_BOLLA.replace("'", "''")+"','"+NUM_CICLO+"','"+NUM_PRINCPL_CESPITE+"','"+NUM_PROGRESSIVO+"','"+NUM_SECONDARIO_CESPITE+"','"+NUM_SEGM_PROFIT+"','"+OGGETTO_DI_COSTO.replace("'", "''")+"','"+PARTNAME+"','"+ORA_CREAZIONE+"','"+ORDINE_CO+"','"+PROFIT_CENTER+"',"+QUANTITA+",'"+SOCIETA+"','"+TESTO_POS_FATT+"','"+TIPO_DES_CONT+"','"+UM_ORDINE_ACQ+"','"+USER_FATTURA+"','"+TIPO_DOC_FATTURA+"','"+DATA_FATTURA+"' FROM DUAL ";
	}
